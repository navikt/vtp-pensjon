    ...
    <Card>
      <Card.Header>
        ...
        <div className="float-right">
          ...
          &nbsp;
          <span className={`badge badge-${statusColor(request.status)}`}>
            {request.status}
          </span>
        </div>
      </Card.Header>
      ...
      <Card.Body>
        <dl className="row">
          ...
          <dd className="col-sm-9 col-md-10">
            {parseDate(request.timestamp).format(dateFormatter)}
          </dd>
 ...
 */
function uniqueAndSorted(
  requestResponses: RequestResponse[]
): RequestResponse[] {
  return Array.from(new Set(requestResponses)).sort((r1, r2) =>
  ...
  useEffect(() => {
    ...
    client.onConnect = () => {
      subscription = client.subscribe("/topic/snitch", (message) => {
        setRequests((state) => {
          let requestResponses = uniqueAndSorted([
            JSON.parse(message.body),
            ...state,
          ]);
          setSelectedRequest((prevState) =>
            prevState != null ? prevState : requestResponses[0]
          );
